...
// no weight/style words, no bare numbers. Fallback: if stripping style words
// leaves nothing, keep the family-category tokens (sans/serif/slab/gothic/script)
// so names like "ITC Serif Gothic" still have something to match on.
function coreTokens($name) {
    ...
    foreach ($w as $t) {
        ...
        if (isStyleWord($t)) {
            $lw = strtolower($t);
            if (in_array($lw, array('sans','serif','slab','gothic','script'), true)) {
                $familyCats[] = $lw;
            }
            continue;
        }
    ...
    if (empty($core) && !empty($familyCats)) {
        return array_values(array_unique($familyCats));
    }
    return $core;
}}
